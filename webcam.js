...
var Webcam = {
		...
		if (this.userMedia) {
			...
			MediaStreamTrack.getSources(function(data){
				...
				for (var i = 0; i < data.length; i++) {
					$("#mediaSources").append("<br>"+data[i].facing);
					if(data[i].facing.indexOf("environment") != -1){
						id = data[i].id;
					}
				}
	...
	snap: function(user_callback, user_canvas) {
		...
		if (this.userMedia) {
			...
			context.drawImage(this.video, 0, 0, $("#my_camera > video")[0].videoWidth/6, $("#my_camera > video")[0].videoHeight/6);
